...
<div class="content-wrapper">
    ...
    <section class="content">
        <!-- Default box -->
        <form action="{{ route('product.store') }}" method="POST" enctype="multipart/form-data" name="productform" id="productform">
            @csrf
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-12">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="compare_price" id="compare_price" class="form-control @error('compare_price') is-invalid @enderror" placeholder="Compare Price" value="{{ old('compare_price') }}">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="barcode" id="barcode" class="form-control @error('barcode') is-invalid @enderror" placeholder="Barcode" value="{{ old('barcode') }}">
                                            @error('barcode')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                            ...
                            <div class="card-body">	
                                ...
                                <div class="mb-3">
                                    <select name="status" id="status" class="form-control @error('status') is-invalid @enderror">
                                        ...
                                        <option value="0" {{ old('status', 1) == 0 ? 'selected' : '' }}>Block</option>
                                    </select>
                                    @error('status')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
...
<script>
    ...
    $(document).ready(function () {
        let myDropzone = new Dropzone("#image", {
            url: "{{ route('temp-images.create') }}", // ✅ هنا رابط المسار الخاص بـ Laravel
            paramName: "image", // الاسم الذي يتوافق مع $request->image في الـ controller
            maxFiles: 1,
            acceptedFiles: "image/*", // فقط الصور مقبولة
            addRemoveLinks: true,
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            success: function (file, response) {
                console.log("✅ تم تحميل الصورة بنجاح:", response);
                $('#image_id').val(response.image_id); // حفظ الـ image_id في الحقل المخفي
            },
            error: function (file, response) {
                console.error("❌ فشل تحميل الصورة:", response);
                Swal.fire("خطأ في التحميل", "حدث خطأ ما!", "error");
            }
        });
    });
</script>
